...
asignar_limites() {
    ...
    echo $cpu_max | sudo tee /sys/fs/cgroup/$usuario/cpu.max
    echo $mem_max | sudo tee /sys/fs/cgroup/$usuario/memory.max
...
if [[ "$usuario" == "x" || "$usuario" == "X" ]]; then
    ...
    echo "Z) Limitar Consumo de GRUPOS"
    echo ""
    ...
    echo "4) Restaurar configuración guardada"
    echo ""
    read -p "Seleccione una opción: " opcion
    if [ "$opcion" == "1" ]; then
